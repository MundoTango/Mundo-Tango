...
-   **AI Integration:**
    ...
    -   **Mr. Blue AI Assistant (RESTORED & ENHANCED - Nov 19, 2025):** Unified interface for Text Chat, Voice Chat, Vibecoding (context-aware code generation), and a **Visual Editor** with iframe preview, element selection, instant DOM updates, change tracking, and voice command integration. Supports seamless mode switching, voice input in all modes, and unified conversation history.
        -   **✅ FIXED: Vibecoding Intent Classification** - Added priority vibecoding keywords ("vibe code", "vibecoding", "vibe coding") to ConversationOrchestrator action intent list (lines 142-158), ensuring requests route to VibeCodingService instead of generic GROQ responses
        -   **✅ FIXED: Context Awareness** - Updated handleQuestion() to accept pageContext parameter and inject page awareness (currentPage, pageTitle, domSnapshot, form fields, buttons, errors) into GROQ system prompt, enabling Mr. Blue to SEE and reference the user's current page
        -   **✅ ENHANCED: System Prompt** - Added vibecoding personality and capabilities advertising to question mode responses, including acknowledgment of current page location
        -   **🔄 KNOWN ISSUE: Chat Memory Database** - /api/mrblue/conversations endpoint returns 500 error due to database query issue, preventing conversation history from loading (authentication working, query execution failing)
