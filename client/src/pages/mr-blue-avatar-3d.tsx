...
import Avatar3D from '@/components/mr-blue/Avatar3D';
import { useEmotionDetection } from '@/hooks/useEmotionDetection';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Smile, Sparkles, ThumbsUp, Brain, MessageCircle, Circle } from 'lucide-react';

type EmotionType = 'idle' | 'happy' | 'surprised' | 'nodding' | 'thinking' | 'speaking';

const emotionButtons: Array<{ emotion: EmotionType; label: string; icon: typeof Smile }> = [
...
export default function MrBlueAvatar3DPage() {
  ...
  const handleEmotionChange = useCallback((detected: EmotionType) => {
    if (autoDetect) {
      setEmotion(detected);
    }
  }, [autoDetect]);
    ...
    <div className="min-h-screen p-4 md:p-8" data-testid="mr-blue-avatar-page">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2" data-testid="emotion-controls">
              {emotionButtons.map(({ emotion: emotionType, label, icon: Icon }) => (
                <Button
                  key={emotionType}
        ...
        <Card>
          ...
          <CardContent className="text-sm text-muted-foreground space-y-2">
            <p>
              Mr. Blue is your AI companion in Mundo Tango. This 3D avatar represents
              the intelligent assistant that helps you navigate the platform, discover
              events, and connect with the global tango community.
            </p>
            <p>
              The avatar responds to emotions and can speak using text-to-speech
              technology. Future versions will include face detection for real-time
              emotion mirroring and personalized interactions.
            </p>
          </CardContent>
        </Card>
      </div>
