...
export default function StreamDetailPage() {
  ...
  if (isLoading) {
    return (
      <PageLayout title={t('pages:streamDetail.pageTitle', 'Live Stream')} showBreadcrumbs>
        <div className="container mx-auto max-w-7xl px-6 py-12">
          <div className="text-center text-muted-foreground">{t('pages:streamDetail.loadingStream', 'Loading stream...')}</div>
        </div>
      </PageLayout>
    );
  }
        ...
        <div className="container mx-auto max-w-7xl px-6 py-12">
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-muted-foreground">{t('pages:streamDetail.streamNotFound', 'Stream not found')}</p>
              <Link href="/live-streams">
                <Button className="mt-4" variant="outline" data-testid="button-back-to-streams">
                  <ArrowLeft className="mr-2 h-4 w-4" />
                  {t('pages:streamDetail.backToStreams', 'Back to Streams')}
                </Button>
              </Link>
            </CardContent>
          </Card>
            ...
            >
              <Link href="/live-streams">
                <Button variant="ghost" size="sm" data-testid="button-back">
                  <ArrowLeft className="mr-2 h-4 w-4" />
                  {t('pages:streamDetail.backToLiveStreams', 'Back to Live Streams')}
                </Button>
              </Link>
            </motion.div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                        ...
                        {stream.isLive && (
                          <div className="absolute top-4 left-4">
                            <Badge variant="destructive" className="flex items-center gap-2 px-3 py-1" data-testid="badge-live">
                              ...
                              <span className="font-semibold">{t('pages:streamDetail.live', 'LIVE')}</span>
                            </Badge>
                          </div>
                        )}
                        ...
                        {stream.viewers !== null && stream.viewers !== undefined && (
                          <div className="absolute top-4 right-4">
                            <Badge className="flex items-center gap-2 bg-black/60 backdrop-blur-sm border-white/20 text-white" data-testid="badge-viewers">
                              ...
                              <span>{stream.viewers?.toLocaleString() || '0'} {t('pages:streamDetail.watching', 'watching')}</span>
                            </Badge>
                          </div>
                        )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-4 pb-4 border-b border-border">
                      ...
                      <Button variant="default" data-testid="button-follow">
                        {t('pages:streamDetail.follow', 'Follow')}
                      </Button>
                    </div>
              ...
              <div className="space-y-6">
                ...
                <FadeInSection delay={0.3}>
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-xl font-serif font-bold">{t('pages:streamDetail.streamInfo', 'Stream Info')}</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      {stream.scheduledDate && !stream.isLive && (
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="text-muted-foreground">
                            {t('pages:streamDetail.scheduled', 'Scheduled')}: {new Date(stream.scheduledDate).toLocaleString()}
                          </span>
                        </div>
                      )}
                      ...
                      {stream.registrations !== null && stream.registrations !== undefined && (
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-muted-foreground">{t('pages:streamDetail.registrations', 'Registrations')}</span>
            ...
            <FadeInSection delay={0.5}>
              <div className="mt-16">
                <h2 className="text-2xl md:text-3xl font-serif font-bold mb-8">
                  {t('pages:streamDetail.moreLiveStreams', 'More Live Streams')}
                </h2>
                <div className="grid gap-6 md:grid-cols-3">
                  ...
                  <Card className="hover-elevate cursor-pointer">
                    ...
                    <CardContent className="pt-4">
                      <Badge variant="outline" className="mb-2">{t('pages:streamDetail.comingSoon', 'Coming Soon')}</Badge>
                      <h3 className="font-serif font-bold">{t('pages:streamDetail.relatedStream', 'Related Stream')}</h3>
                    </CardContent>
                  </Card>
                </div>
              </div>
