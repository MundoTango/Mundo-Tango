...
import { DollarSign, Clock, Save, ArrowLeft, Loader2 } from "lucide-react";
...
export default function EditEventPage() {
  ...
  useEffect(() => {
    if (event) {
      ...
      form.reset({
        ...
        startDate: formatDateForInput(event.startDate || event.date),
      ...
      if (event.city) {
        const tz = getTimezoneFromCity(event.city);
        setUserTimezone(tz);
      }
    }
  }, [event, form]);
  ...
  if (!user) {
    return (
      <SelfHealingErrorBoundary pageName="Edit Event" fallbackRoute="/events">
        <>
          <SEO title="Edit Event" description="Edit your tango event" />
  ...
  if (error || !event) {
    return (
      <SelfHealingErrorBoundary pageName="Edit Event" fallbackRoute="/events">
        <>
          <SEO title="Event Not Found" description="Event not found" />
          <div className="max-w-3xl mx-auto px-6 py-12">
            <Card>
              <CardContent className="pt-6 text-center">
  ...
  const isOrganizer = event.organizerId === user.id || event.organizer?.id === user.id;
  if (!isOrganizer) {
    return (
      <SelfHealingErrorBoundary pageName="Edit Event" fallbackRoute="/events">
        <>
          <SEO title="Not Authorized" description="Not authorized to edit this event" />
          <div className="max-w-3xl mx-auto px-6 py-12">
            <Card>
              <CardContent className="pt-6 text-center">
  ...
  return (
    <SelfHealingErrorBoundary pageName="Edit Event" fallbackRoute="/events">
      <>
        <SEO 
          title={`Edit ${event.title} - Mundo Tango`}
          description="Edit your tango event details"
        />
        <PageLayout>
          <div className="max-w-3xl mx-auto px-6 py-12">
            <div className="mb-8">
