...
export const insertUiTestScenarioSchema = createInsertSchema(uiTestScenarios)
  .omit({ id: true, createdAt: true, updatedAt: true })
  .refine((data) => data.estimatedMinutes > 0, {
...
export const uiTestResults = pgTable("ui_test_results", {
  id: serial("id").primaryKey(),
  scenarioId: integer("scenario_id").notNull().references(() => uiTestScenarios.id, { onDelete: "cascade" }),
  userId: integer("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  sessionId: varchar("session_id", { length: 100 }).notNull(),
  ...
  scenarioIdx: index("ui_test_results_scenario_idx").on(table.scenarioId),
  userIdx: index("ui_test_results_user_idx").on(table.userId),
  sessionIdx: index("ui_test_results_session_idx").on(table.sessionId),
  completedIdx: index("ui_test_results_completed_idx").on(table.completed),
}));
...
export const volunteerStats = pgTable("volunteer_stats", {
  ...
  bugsFound: integer("bugs_found").default(0),
  averageDifficultyRating: real("average_difficulty_rating"),
  skillLevel: varchar("skill_level", { length: 20 }).default("beginner"),
  lastSessionAt: timestamp("last_session_at"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => ({
  userIdx: index("volunteer_stats_user_idx").on(table.userId),
  skillLevelIdx: index("volunteer_stats_skill_level_idx").on(table.skillLevel),
}));
