...
export class AgentCardRegistry {
  ...
  async registerAgent(card: AgentCard): Promise<void> {
    ...
    // Skip database persistence if DATABASE_URL is not configured
    const dbUrl = process.env.DATABASE_URL || '';
    if (!dbUrl || dbUrl.includes('YOUR_PASSWORD') || dbUrl.includes('YOUR_HOST')) {
      // Database not configured - skip persistence (registry still works in-memory)
      return;
    }
