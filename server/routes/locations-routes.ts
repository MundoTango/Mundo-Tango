...
// POST /api/locations/search - Search locations via OpenStreetMap Nominatim API (for long addresses)
...
router.post("/search", async (req, res: Response) => {
  try {
    ...
    // STEP 3: Fall back to Nominatim API only for less common locations
    console.log(`[LocationSearch] Calling Nominatim for "${query}"...`);
    const addressDetailsParam = addressdetails ? '&addressdetails=1' : '&addressdetails=0';
    const nominatimUrl = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}&format=json&limit=6${addressDetailsParam}`;
