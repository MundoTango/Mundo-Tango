 ...
 * Check if current user is a member of this city
 */
router.get("/:id/membership", async (req: Request, res: Response) => {
  try {
    ...
    // Check city_members table first
    const [cityMember] = await db
      .select()
      .from(cityMembers)
      .where(and(
        eq(cityMembers.cityId, id),
    ...
    // Fall back to checking group membership via legacyGroupId
    const [city] = await db
      .select()
      .from(cities)
      .where(eq(cities.id, id))
    ...
    if (city?.legacyGroupId) {
      const [groupMember] = await db
        ...
        .from(groupMembers)
        .where(and(
          eq(groupMembers.groupId, city.legacyGroupId),
          eq(groupMembers.userId, userId),
          eq(groupMembers.status, 'active')
        ))
        .limit(1);
      
      return res.json({ isMember: !!groupMember });
    }
...
router.post("/:id/join", async (req: Request, res: Response) => {
  try {
    ...
    // If city has legacyGroupId, join the group instead
    if (city.legacyGroupId) {
      ...
      await db
        .update(cities)
        .set({ memberCount: sql`${cities.memberCount} + 1` })
        .where(eq(cities.id, id));
      
      return res.json({ success: true, message: "Joined city community" });
    }
    
    // Otherwise use city_members table
    const [existingMember] = await db
      .select()
      .from(cityMembers)
      .where(and(
    ...
    if (existingMember) {
      if (existingMember.status !== 'active') {
        await db
          .update(cityMembers)
          .set({ status: 'active' })
          .where(eq(cityMembers.id, existingMember.id));
      }
    } else {
      await db
        .insert(cityMembers)
    ...
    res.json({ success: true, message: "Joined city community" });
  } catch (error) {
...
/**
 * DELETE /api/cities/:id/leave
 * Leave a city community
 */
router.delete("/:id/leave", async (req: Request, res: Response) => {
  try {
    ...
    // If city has legacyGroupId, leave the group
    if (city.legacyGroupId) {
      await db
        .update(groupMembers)
        .set({ status: 'inactive' })
    ...
    // Also update city_members if exists
    await db
      .update(cityMembers)
      .set({ status: 'inactive' })
      .where(and(
    ...
    await db
      .update(cities)
      .set({ memberCount: sql`GREATEST(0, ${cities.memberCount} - 1)` })
      .where(eq(cities.id, id));
    
    res.json({ success: true, message: "Left city community" });
  } catch (error) {
    console.error("[CityLeave] Error:", error);
    res.status(500).json({ error: "Failed to leave city" });
  }
});
