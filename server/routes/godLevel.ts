...
import {
  ...
  requireAdmin,
  GodLevelRequest,
} from "../middleware/requireGodLevel";
...
router.post("/request", async (req: GodLevelRequest, res) => {
  try {
    ...
    const result = await approvalService.requestGodLevelAccess(
      req.user.id,
      reason,
    );
  ...
  async (req: GodLevelRequest, res) => {
    try {
      ...
      const month = req.query.month
        ...
        : new Date();
      const spend = await costTrackerService.getMonthlySpend(
        req.user.id,
        month,
      );
      const breakdown = await costTrackerService.getUserCostBreakdown(
        req.user.id,
        month,
      );
  ...
  async (req: GodLevelRequest, res) => {
    try {
      ...
      const months = req.query.months
        ...
        : 3;
      const history = await costTrackerService.getUserSpendHistory(
        req.user.id,
        months,
      );
...
router.get(
  ...
  async (req: GodLevelRequest, res) => {
    try {
      ...
      const prediction = await costTrackerService.predictMonthlyBill(
        req.user.id,
      );
      res.json({ prediction });
    } catch (error: any) {
...
// Trigger event scraping from all sources
router.post(
  "/admin/trigger-scrape",
  // requireAdmin,
  async (req: GodLevelRequest, res) => {
    try {
      const { scrapingOrchestrator } = await import(
        "../agents/scraping/masterOrchestrator"
      );

      // Start scraping in background
      scrapingOrchestrator.orchestrate().catch((err) => {
        console.error("Scraping error:", err);
      });

      res.json({
        success: true,
        message: "Scraping started for all 194 sources",
      });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
